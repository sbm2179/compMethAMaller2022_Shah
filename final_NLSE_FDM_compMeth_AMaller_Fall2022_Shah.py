...
tau=100e-12 #Pulse width in second 
...
def comp_A(A):
    for i in range(0,Nx-1):
        for j in range(0,Nt-1):
            A[i+1][j]=A[i][j]-((1j*b2/2)*(dx/dt**2)*(A[i][j+1]-2*A[i][j]+A[i][j-1]))+((1j*gamma*I)*A[i][j])
    return A
...
for i in range(0,Nx):
    plt.semilogy(t,np.absolute(A_dfm[i]),label=str((1e-7)*i)+" meter")
    plt.legend()
